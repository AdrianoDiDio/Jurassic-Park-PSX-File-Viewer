...
void FileDialogRender(FileDialog_t *FileDialog)
{
    ...
    if (IGFD_DisplayDialog(FileDialog->Window, FileDialog->Key, 
        ...
        if (IGFD_IsOk(FileDialog->Window)) {
                FileName = IGFD_GetFilePathName(FileDialog->Window,IGFD_ResultMode_OverwriteFileExt);
...
void FileDialogOpenWithUserData(FileDialog_t *FileDialog,const char *Path,void *UserData)
{
    ...
    struct IGFD_FileDialog_Config FileDialogConfig;
    ...
    FileDialogConfig = IGFD_FileDialog_Config_Get();
    FileDialogConfig.path = StartingPath;
    FileDialogConfig.userDatas = UserData;
    FileDialogConfig.countSelectionMax = 1;
    FileDialogConfig.flags = ImGuiFileDialogFlags_DontShowHiddenFiles | ImGuiFileDialogFlags_CaseInsensitiveExtention;
    IGFD_OpenDialog(FileDialog->Window,FileDialog->Key,FileDialog->WindowTitle,FileDialog->Filters,FileDialogConfig);
}
/*
